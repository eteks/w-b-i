...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $user_mobile = "61".$_POST['username'];
    $mobile = "61".$_POST['mobile'];
    $mob = $_POST['mobile'];
    ...
    if($myusername != '' && $password != '') {
        ...
        $sql = mysql_query("SELECT * FROM ezikey_users WHERE (user_name = '$myusername' OR mobile_number = '$user_mobile') AND user_password = '$mypassword' AND user_status=1");
        ...
        if ($count == 1) {
            ...
            $already_sql = mysql_query("SELECT * FROM ezikey_users WHERE (user_name = '$myusername' OR mobile_number = '$user_mobile') AND user_password = '$mypassword'");
            $already_sql_count = mysql_num_rows($already_sql);

            if ($already_sql_count == 1) {
                $user_array = mysql_fetch_array($already_sql);
                $_SESSION['approval_status'] = 1;
                $_SESSION['user_id'] = $user_array['user_id'];
                header("location: approval.php");
            }
            else if ($mob == '') {
                $error = "Invalid login credentials. Please signup to enter mobile number.";
            }
            else {
                $already_username_query = mysql_query("SELECT * FROM ezikey_users WHERE user_name = '$myusername'");
                $already_username_count = mysql_num_rows($already_username_query);
                if($already_username_count == 1) {
                    $error = "Username already exists."; 
                }
                else {
                    $already_mobile_query = mysql_query("SELECT * FROM ezikey_users WHERE mobile_number = '$mobile'");
                    $already_mobile_count = mysql_num_rows($already_mobile_query);
                    if($already_mobile_count == 1) {
                        ...
                        */
                        $msg = "Hello,+A+new+user+".$myusername."+is+registered+with+us+by+".$mobile.".+Please+send+approval+status+to+the+user+very+soon.";
                        ...
                        if($response) {
                            ...
                            mysql_query("INSERT INTO ezikey_users (user_name,mobile_number,user_password,message_id,user_token,user_status) VALUES ('$myusername','$mobile','$mypassword','$message_id','$token',0)");
                    ...
                    <div class="middle">
                        <div class="col-lg-6 col-lg-offset-4">
                            ...
                            <form class="form-horizontal min" action="#" method="post">
                                <div class="col-lg-6 margin_section col-md-6 col-sm-6 col-xs-12">
                                    <div class="form-group">
                                        <input type="text" class="form-control login_field" name="username" placeholder="Username / Mobile Number" maxlength="20" autocomplete="off" required />
                                    </div>
                                    <div class="form-group">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                    <p style="position: absolute; left: 0px;"> For signup ? </p>
